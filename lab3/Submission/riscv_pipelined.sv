...
module riscv(input  logic        clk, reset,
   ...
   controller c(clk, reset,
		...
		MemWriteM, RegWriteM, 
		RegWriteW, ResultSrcW, JumpD, MemStrobeM);
...
module controller(input  logic		 clk, reset,
   ...
   maindec md(opD, ResultSrcD, MemWriteD, BranchD,
              ALUSrcAD, ALUSrcBD, RegWriteD, JumpD, ImmSrcD, ALUOpD, MemStrobeD);
   ...
   floprc #(16) controlregE(clk, reset, FlushE,
                            {RegWriteD, ResultSrcD, MemWriteD, JumpD, BranchD, ALUControlD, ALUSrcAD, ALUSrcBD, funct3D, MemStrobeD},
                            {RegWriteE, ResultSrcE, MemWriteE, JumpE, BranchE, ALUControlE, ALUSrcAE, ALUSrcBE, funct3E, MemStrobeE});
    assign {Zero, N, V, C} = BranchLogicE;

    assign BranchCondition = (Zero & ~funct3E[0] &  ~funct3E[1] & ~funct3E[2] ) |
                              (~Zero & funct3E[0] &  ~funct3E[1] & ~funct3E[2] ) |
                              ((N ^ V) & ~funct3E[0] &  ~funct3E[1] &funct3E[2] ) |
                              (~(N ^ V) & funct3E[0] &  ~funct3E[1] & funct3E[2] ) |
                              (~C & ~funct3E[0] &  funct3E[1] & ~funct3E[2] ) |
                              (C & funct3E[0] &  funct3E[1] & funct3E[2] );
